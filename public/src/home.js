...
function getBooksBorrowedCount(books) {
  // Use the filter() method to filter the books array to include only the books that have not been returned
  const borrowedBooks = books.filter(book => !book.borrows[0].returned);
  // Return the length of the filtered array, which represents the number of books that are currently checked out
